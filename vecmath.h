...
public:
    ...
    vec2(){}
    vec2(T x, T y){elem[0]=x; elem[1]=y;}
    vec2(T x){elem[0]=elem[1]=x;}

	T operator()(int x) const {return elem[x];}
	T &operator()(int x) {return elem[x];}

    vec2 operator *(const T x) const {vec2 res(*this); res.elem[0]*=x; res.elem[1]*=x; return res;}
    vec2 operator /(const T x) const {vec2 res(*this); res.elem[0]/=x; res.elem[1]/=x; return res;}
    ...
    vec2 &operator *=(const T x) {elem[0]*=x; elem[1]*=x; return (*this);}
    vec2 &operator /=(const T x) {elem[0]/=x; elem[1]/=x; return (*this);}
	...
	T Magnitude(void) const {
		return(float(sqrt((elem[0] * elem[0]) + (elem[1] * elem[1]))));
	}
	T Magnitude2(void) const {return((elem[0]*elem[0])+(elem[1]*elem[1]));}
	T Normalise(void) { T x = Magnitude(); elem[0]/=x; elem[1]/=x; return x;}
    vec2 Normalised(void) const {vec2 res(*this); res.Normalise(); return res;}

	T Dot(const vec2 &x) const {return ( (elem[0]*x.elem[0]) + (elem[1]*x.elem[1]) );}
};
